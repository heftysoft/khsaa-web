...
export interface MembershipTier {
  id: string;
  name: string;
  type: "GENERAL" | "HONORABLE" | "LIFETIME";
...
export function MembershipDetails({
  ...
  // Add this useEffect to fetch payment info
  useEffect(() => {
  ...
  const handleCreateMembership = async () => {
    ...
    // Check if payment info is required (required for all except HONORABLE)
    const isPaymentRequired = selectedTier?.type !== "HONORABLE";

    // Validate payment info if required
    if (
      isPaymentRequired &&
      (!formData.transactionId || !formData.paymentDetails)
    ) {
      toast.error("Error", {
        description: "Please provide payment information",
      });
      return;
    }
        ...
        <Card className="bg-muted">
          <CardContent className="pt-6">
            <div className="flex items-center gap-4 text-muted-foreground">
              ...
              <p>
                Your previous membership was cancelled. Choose a new plan to
                continue.
              </p>
            </div>
          </CardContent>
            ...
            <CardContent>
              <div className="space-y-6">
                <MembershipTiers
                  tiers={tiers}
                  onSelect={(id) =>
                    setFormData((prev: any) => ({ ...prev, tierId: id }))
                  }
                ...
                {formData.tierId && (
                  <div className="space-y-4 pt-6 border-t">
                    <h3 className="text-lg font-medium">
                      {tiers.find((tier) => tier.id === formData.tierId)
                        ?.type === "HONORABLE"
                      ...
                      <Card className="bg-muted">
                        ...
                        <CardContent className="space-y-4">
                          ...
                          {formData.paymentMethod === "BKASH" && (
                            <div className="whitespace-pre-wrap text-sm">
                              {paymentInfo.bkashInfo}
                            </div>
                          )}
                          {formData.paymentMethod === "NAGAD" && (
                            <div className="whitespace-pre-wrap text-sm">
                              {paymentInfo.nagadInfo}
                            </div>
                          )}
                          {formData.paymentMethod === "ROCKET" && (
                            <div className="whitespace-pre-wrap text-sm">
                              {paymentInfo.rocketInfo}
                            </div>
                          )}
                        </CardContent>
                      </Card>
            ...
            <CardFooter className="flex justify-end">
              <Button
                ...
                disabled={
                  !formData.tierId ||
                  isLoading ||
                  (tiers.find((tier) => tier.id === formData.tierId)?.type !==
                    "HONORABLE" &&
                    (!formData.transactionId || !formData.paymentDetails))
                }
              >
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Badge
                    variant="secondary"
                    className={`${getStatusColor(membership.status)} text-white`}
                  >
                    {membership.status}
                  </Badge>
                </div>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        Are you sure you want to cancel your membership? This
